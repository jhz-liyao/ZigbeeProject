...
void SendHearBeat(void){
  ...
  Protocol_Send(WATER_HEARTBEAT_PROTOCOL, &Heartbeat_P, sizeof(HEARTBEAT_PROTOCOL_T));
}
...
//          byte sentTransID = afDataConfirm->transID;(void)sentTransID;  
        ...
        case ZDO_STATE_CHANGE:
          ...
          if ( (App_NwkState == DEV_ZB_COORD) || (App_NwkState == DEV_ROUTER) || (App_NwkState == DEV_END_DEVICE) ){ 
            ...
            osal_start_timerEx( App_TaskId, WATER_MACHINE_STATE_CHECK_EVT, WATER_MACHINE_STATE_CHECK_TIMEOUT );
          }
          break; 
        default:
          break;
  ...
  if( events & WATER_MACHINE_STATE_CHECK_EVT){
    State_Check();
    osal_start_timerEx( App_TaskId, WATER_MACHINE_STATE_CHECK_EVT, 100 );
