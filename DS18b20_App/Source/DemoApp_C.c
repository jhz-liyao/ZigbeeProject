...
uint16 Demo_ProcessEvent( uint8 task_id, uint16 events )
{
      ...
      switch ( MSGpkt->hdr.event )
      {
        ...
        case ZDO_STATE_CHANGE:
          ...
          if ( (Demo_NwkState == DEV_ZB_COORD) ||
               (Demo_NwkState == DEV_ROUTER) ||
               (Demo_NwkState == DEV_END_DEVICE) )
          {
            // Start sending "the" message in a regular interval.
            osal_start_timerEx( Demo_TaskID,
                                DEMO_SEND_MSG_EVT,
                                txMsgDelay );
          }
  ...
  if ( events & RCV_CNT_EVT ){
    sync_cnt();
      printf("remoteAddr:%X %X\r\n",remot_ShortAddr & 0xff,remot_ShortAddr>>8);
      Demo_DstAddr.addr.shortAddr = remot_ShortAddr;
      AF_DataRequest( &Demo_DstAddr, &Demo_epDesc,
                       DEMO_CLUSTERID,
                       1,
                       (byte *)&rcv_cnt,
                       &Demo_TransID,
                       AF_DISCV_ROUTE, AF_DEFAULT_RADIUS );
    osal_start_timerEx( Demo_TaskID, RCV_CNT_EVT, 1000 );
...
static void Demo_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  switch ( pkt->clusterId )
  {
    ...
    remot_ShortAddr = *((uint16*)(pkt->cmd.Data+1)); 
