...
//                                                               pr->pi.tail);
			...
			case 8: //处理帧尾 帧类型和长度进行匹配 
						...
						for(j = 0; j < RECV_PROTOCOL_NUM; j++){
							if(	src_board_action ==  Recv_Desc_P[j].ModuleAction &&//目标板匹配,动作匹配 
									pr->pi.paralen == Recv_Desc_P[j].ProtocolSize)//帧长度匹配
							{
								pr->pi.handle = Recv_Desc_P[j].handle;
								pr->pi.check = Recv_Desc_P[j].check;
								pr->pi.Desc_T = &Recv_Desc_P[j];
								break;
							}
						}
						if(j == RECV_PROTOCOL_NUM){//校验不通过
							_clean_recv_buf(pr);
...
/****************************************************
	...
	作者:		liyao 2015年9月8日14:10:51      
****************************************************/
void ProtocolFrame_Init(SEND_INTERFACE SendHandle){
