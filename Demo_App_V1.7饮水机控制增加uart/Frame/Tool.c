...
void UART1_CallBack( uint8 port, uint8 event ){
  uint8 cnt = 0, flag_FD = 0, flag_F8 = 0, i; 
  uint8 data[MT_UART_DEFAULT_MAX_RX_BUFF] = {0}; 
  cnt = Hal_UART_RxBufLen(port);
  cnt = HalUARTRead (port, data, cnt);
//  for(i = 0;i < cnt; i++){
//    HalUARTWrite(HAL_UART_PORT_0, &data[i], 1); 
//  }
  for(i = 0; i < cnt; i++){
    if(data[i] == 0xfd)
      flag_FD = i;
  ...
  if(flag_FD != 0 && flag_F8 != 0)
    UART1_Resolver->Protocol_Put(UART1_Resolver,&data[flag_FD], cnt - flag_FD);  
  ESP8266_Poll(data, cnt);
  //BrodcastData(data, cnt);
//  HalUARTWrite(port, data, cnt);
}
